...
def _jobs(strategies: list[str]) -> list[Job]:
    return enumerate_jobs(strategies, CATEGORIES, FIT_SEEDS, SEARCH_SEEDS)


def test_grid_job_count_is_category_by_fit_seed() -> None:
    # grid ignores the search seed, so it collapses to one job per (category, fit-seed).
    jobs = _jobs(["grid"])
    assert len(jobs) == len(CATEGORIES) * len(FIT_SEEDS)
    assert all(job.search_seed is None for job in jobs)


@pytest.mark.parametrize("strategy", ["random", "bayes"])
def test_search_strategy_job_count_includes_search_seeds(strategy: str) -> None:
    jobs = _jobs([strategy])
    assert len(jobs) == len(CATEGORIES) * len(FIT_SEEDS) * len(SEARCH_SEEDS)
    assert {job.search_seed for job in jobs} == set(SEARCH_SEEDS)
...
def _build_one(strategy: str, *, fit_seed: int, search_seed: int | None) -> Job:
    seeds = [search_seed] if search_seed is not None else SEARCH_SEEDS
    return next(
        job
        for job in enumerate_jobs([strategy], ["Books"], [fit_seed], seeds)
        if job.fit_seed == fit_seed and job.search_seed == search_seed
    )


def test_pending_jobs_drops_existing_results(tmp_path: Path) -> None:
    jobs = enumerate_jobs(["grid"], CATEGORIES, FIT_SEEDS, SEARCH_SEEDS, results_dir=tmp_path)
    done = jobs[0]
...
def test_partition_covers_every_job_disjointly_and_evenly() -> None:
    jobs = _jobs(["grid", "random", "bayes"])
    n_boxes = 4
    boxes = partition(jobs, n_boxes)
    assert len(boxes) == n_boxes
    # Every job lands in exactly one box (a dead box must not silently drop work).
    flattened = [job for box in boxes for job in box]
    assert sorted(flattened, key=id) == sorted(jobs, key=id)
...
def test_partition_rejects_non_positive_box_count() -> None:
    with pytest.raises(ValueError, match="n_boxes must be >= 1"):
        partition(_jobs(["grid"]), 0)


def test_render_box_script_guards_and_runs_each_job() -> None:
    jobs = _jobs(["random"])[:2]
    script = render_box_script(jobs)
    assert script.startswith("#!/usr/bin/env bash\nset -euo pipefail")
    # Each job's command and its skip-if-exists guard both appear.
    for job in jobs:
        assert shlex.join(job.command) in script
        assert f"[ -f {job.out_path}" in script
    assert script.count("mkdir -p") == len(jobs)
...
def test_dispatch_plan_run_executes_jobs_locally(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    # --run shells out to each pending job's command instead of only writing scripts.
    calls: list[list[str]] = []
    monkeypatch.setattr(
    ...
    cli._COMMANDS[args.command](args)
    # One subprocess call per planned grid job (2 fit seeds x 1 category).
    assert len(calls) == 2
