...
"""The benchmark's selected Amazon Reviews 2023 categories (HF hub names) -- see the README's
"Datasets" section for the selection criteria and per-category stats.

The list ``preprocess --all`` iterates. ``build_dataset`` takes any category by name, so an
unselected one (e.g. Gift_Cards for a quick smoke test) can still be built on demand with
